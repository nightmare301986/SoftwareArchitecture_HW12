...
import com.example.Connection.Message;
import com.example.Connection.MessageType;

public class Server {
	private ServerSocket serverSocket;
	private static ViewGuiServer gui; // объект класса View
	private static ModelGuiServer model; // объект класса Model
	...
	// метод, в котором в бесконечном цикле сервер принимает новое сокетное
	...
	// метод рассылающий заданное сообщение всем клиентам из мап
	...
	public static void main(String[] args) {
		...
		gui.initFrameServer();
		// цикл снизу ждет true от флага isServerStart (при старте сервера в методе
		// startServer устанавливается в true)
		// после чего запускается бесконечный цикл принятия подключения от клиента в
		// методе acceptServer
	...
	// класс-поток, который запускается при принятии сервером нового сокетного
	...
	private class ServerThread extends Thread {
		...
		// метод реализующий запрос сервера у клиента имени и добавлении имени в мап
		...
		private void messagingBetweenUsers(Connection connection, String userName) {
			while (true) {
				try {
					...
					// приняли сообщение от клиента: если тип сообщения TEXT_MESSAGE, то пересылаем
					// его всем пользователям
					...
					// если тип сообщения DISABLE_USER, то рассылаем всем пользователям,
					// что данный пользователь покинул чат,
		...
		public void run() {
			...
			try {
				...
				// регистрируем, запускаем цикл обмена сообщениями между пользователями
